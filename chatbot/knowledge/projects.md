...
## VicTouch (Volgistics)
Scott independently rewrote VicTouch, Volgistics' flagship application generating roughly $1M in annual recurring revenue, from the ground up — architecting and building both the Angular frontend and the backend services himself. He shipped a production beta in 6 months and fully replaced the legacy system within 12 months, all without AI-assisted coding tools. VicTouch is a volunteer clock-in/clock-out system with messaging, scheduling, and touchless QR code sign-in, and was the first app Volgistics released to the App Store and Play Store — also the company's first Angular web application and first native application. He implemented continuous integration and deployment processes and worked with non-technical volunteer experts to ensure customer requirements were fulfilled. Built with Angular SPA frontend, Capacitor for cross-platform deployment, Windows Server with Delphi backend, and BTrieve database. Deployed via GitLab CI/CD.
